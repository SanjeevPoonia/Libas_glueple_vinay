...
class ApplyAttendanceCorrectionScreen extends StatefulWidget{
  final String alDate;
  final String alCheckIn;
  final String alcheckOut;
  ApplyAttendanceCorrectionScreen({required this.alDate,required this.alCheckIn,required this.alcheckOut});
  _applyCorrection createState()=>_applyCorrection();
}
class _applyCorrection extends State<ApplyAttendanceCorrectionScreen>{
  ...
  _checkTheValidation(){
     if(applyDate.isEmpty){
      ...
      Toast.show("Please Select Check Out Time",
       ...
       Toast.show("Please enter the attendance correction Reason.",
           duration: Toast.lengthLong,
           gravity: Toast.bottom,
           backgroundColor: Colors.red);
     }else{
  ...
  _applyCorrectionOnServer()async{
    ...
    var requestModel = {
      "emp_id": empId,
      "date":applyDate,
      "corrected_check_in_time":checkInTime,
      "corrected_check_out_time":checkOutTime,
      "correction_reason":reasonController.text.toString(),
      "type":"both"

    };
    ApiBaseHelper helper = ApiBaseHelper();
    var response = await helper.postAPIWithHeader(baseUrl, 'admin/employeeAttendanceCorrectionHikvision',requestModel,context, token);
